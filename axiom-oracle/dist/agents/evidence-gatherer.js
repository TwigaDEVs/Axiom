 ...
 */
async function searchGNews(query, timeWindow, maxResults = 5) {
    if (!GNEWS_API_KEY)
        return [];
    try {
 ...
 */
async function searchGoogleNewsRSS(query, timeWindow, maxResults = 5) {
    try {
...
function extractLink(block) {
    ...
    if (standard && standard.startsWith("http"))
        return standard;
...
function classifySourceType(sourceName) {
    ...
    if (wireServices.some((w) => name.includes(w)))
        return "wire_service";
    if (official.some((o) => name.includes(o)))
        return "official";
    if (mainstream.some((m) => name.includes(m)))
        return "mainstream_news";
    if (tradePress.some((t) => name.includes(t)))
        return "trade_press";
    return "unknown";
}
// ─── Deduplication ────────────────────────────────────────────────
...
//# sourceMappingURL=evidence-gatherer.js.map