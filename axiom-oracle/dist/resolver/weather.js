...
function isRecentPast(dateStr, daysBack = 7) {
    const date = new Date(dateStr);
    const now = new Date();
...
class WeatherFetcher {
    async fetch(spec) {
        try {
            ...
            const metric = spec.metric || "temperature";
            const unit = spec.unit || "fahrenheit";
            const measurementType = spec.measurement_type || "max";
