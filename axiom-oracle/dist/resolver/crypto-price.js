...
exports.CryptoPriceFetcher = void 0;
...
class CryptoPriceFetcher {
     ...
     */
    async fetchSpot(symbol, resolutionTime) {
        try {
     ...
     */
    async fetchHistorical(symbol, timestampMs) {
        try {
     ...
     */
    async fetchTWAP(symbol, window = "1h") {
        try {
            ...
            const twap = closePrices.reduce((a, b) => a + b, 0) / closePrices.length;
            return {
                success: true,
                data: {
                    symbol,
     ...
     */
    async fetch(spec) {
        const pair = spec.pair || "BTC/USD";
        const symbol = toBinanceSymbol(pair);
        const method = spec.aggregation_method || "SPOT";
        const resolutionTime = spec.resolution_time;
        if (method === "TWAP") {
            const window = spec.window || "1h";
            return this.fetchTWAP(symbol, window);
        }
        return this.fetchSpot(symbol, resolutionTime);
    }
}
exports.CryptoPriceFetcher = CryptoPriceFetcher;
